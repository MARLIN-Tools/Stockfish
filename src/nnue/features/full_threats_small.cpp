...
constexpr auto init_threat_offsets() {
    ...
    for (Piece piece : AllPieces)
    {
        ...
        cumulativeOffset += numValidTargetsSmall[pieceIdx] * cumulativePieceOffset;
    }
...
constexpr auto init_index_luts() {
    ...
    for (Piece attacker : AllPieces)
    {
        for (Piece attacked : AllPieces)
        {
            ...
            int  map           = FullThreatsSmall::map[attackerType - 1][attackedType - 1];
            ...
            IndexType feature  = helper_offsets[attacker].cumulativeOffset
                              + (color_of(attacked) * (numValidTargetsSmall[attacker] / 2) + map)
                                  * helper_offsets[attacker].cumulativePieceOffset;

            bool excluded                  = map < 0;
            indices[attacker][attacked][0] = excluded ? FullThreatsSmall::Dimensions : feature;
            indices[attacker][attacked][1] =
              excluded || semi_excluded ? FullThreatsSmall::Dimensions : feature;
        }
    }
...
inline sf_always_inline IndexType FullThreatsSmall::make_index(
...
void FullThreatsSmall::append_active_indices(Color perspective,
                                             const Position& pos,
                                             IndexList& active) {
...
void FullThreatsSmall::append_changed_indices(Color                   perspective,
                                              Square                  ksq,
                                              const DiffType&         diff,
                                              IndexList&              removed,
                                              IndexList&              added,
...
bool FullThreatsSmall::requires_refresh(const DiffType& diff, Color perspective) {
